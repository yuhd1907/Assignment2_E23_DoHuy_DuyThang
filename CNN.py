...
# Define the CNN class with 3 convolution layers
class CNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(64 * 4 * 4, 512)                    # Fully connected layer: 64*4*4 -> 512
        self.fc2 = nn.Linear(512, 10)                            # Output layer: 512 -> 10 classes
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 64 * 4 * 4)              # Flatten to (batch_size, 1024)
        x = self.relu(self.fc1(x))
        x = self.fc2(x)
        return x

# Function to compute moving average for smoothing curves
...
def main():
    ...
    # Evaluate and save predicted images
    model.eval()
    with torch.no_grad():
        ...
        # Combine predicted images into one
